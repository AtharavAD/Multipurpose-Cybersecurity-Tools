...
label = tk.Label(window, text="NMAP PORT SCANNER", font=('Inter', 30, 'bold'), fg='#000f46')
...
def open_specific_file2():
    ...
    # Specify the path to the Python script you want to open
    file_path = "D:\\Multipurpose Cybersecurity Tool\\Tools\\New_NMAP.py"
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Introduction_text = """Nmap (Network Mapper) is a network scanner created by Gordon Lyon. Nmap is
...
vulnerability detection, and other features. Nmap can adapt to network
conditions including latency and congestion during a scan.

Nmap started as a Linux utility and was ported to other systems including
Windows, macOS, and BSD. It is most popular on Linux, followed by Windows.
"""

Steps_text = """STEP 1: Click on the NMAP tool button, a new window will pop up on screen.

STEP 2: Enter the address of the device and It will check if it is valid or not.

STEP 3: An option list will be displayed containing 8 different categories of
             network scanning, select one option from 1 to 8 and press the enter key.

STEP 4: It will display the output of that respective option in detail format,
             the system will display the option list again.

STEP 5: If yes then enter the details and then follow previous step similarly.

STEP 6: To stop, simply select the option 8 as it is the EXIT option.
"""

Insights_text = """Advantages:

1. Thorough Network Discovery: Reveals hosts, services, and OS details for
    network mapping.
2. Flexible & Extensible: Offers diverse scanning options and customization
    through NSE.
3. Security Assessment: Helps in identifying weaknesses and securing networks.


Disadvantages:

1. Legal Risks: Unauthorized use can lead to legal consequences.
2. Detection by Firewalls/IDS: Can be detected and blocked by advanced security
    systems.
3. Complexity: Requires knowledge and expertise for effective utilization."""

Application_text = """Applications:

1. Network Mapping: Identifying hosts, services, and open ports on a network.
2. Security Auditing: Assessing network security and vulnerabilities.
3. Network Troubleshooting: Diagnosing network connectivity issues and
    configurations."""

Result = PhotoImage(file= 'D:\\Multipurpose Cybersecurity Tool\\UI\\03_tools manual\\Result_Images\\NMAP2.png')
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
