...
label = tk.Label(window, text="          EMAIL BOMBER", font=('Inter', 30, 'bold'), fg='#000f46')
...
def open_specific_file2():
    ...
    # Specify the path to the Python script you want to open
    file_path = "D:\\Multipurpose Cybersecurity Tool\\Tools\\Email_Bomber.py"
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Introduction_text = """
An Email Bomber Tool is a type of software or program designed to flood an 
individual's or organization's email inbox by sending a large number of emails in a 
short period. This tool can be misused for malicious purposes, causing 
inconvenience, disruption, or potential harm to the targeted recipient. The excess 
of emails generated by such tools can overload the email server, making it 
challenging for the recipient to manage their inbox and potentially causing 
legitimate emails to be overlooked. Engaging in email bombing activities is widely 
considered unethical and may lead to legal consequences, as it can disrupt normal 
communication and violate email service terms of use.

Imagine you have a friend who keeps sending you a massive pile of letters, filling 
up your mailbox so much that you can't find anything else. An Email Bomber Tool is 
like that, but for your email inbox. It sends a ton of emails to someone's email 
address, overwhelming their inbox and making it difficult for them to find important 
messages. It's like a virtual flood of emails!
"""

Steps_text = """STEP 1: Click on the Email Bomber tool button, a new window will pop up on 
             screen.

STEP 2: Enter the target email in the dialogue box.

STEP 3: Enter the bomb mode select option from 1 to 4.

STEP 4: Enter the amount of bomb (any numeric value).

STEP 5: The details of selected option will be displayed.

STEP 6: Set up the email by filling the required information like subject, message, 
             from email etc. the attack will take place and get displayed on screen.
...
Insights_text = """Advantages:
 ...
 1. Can disrupt email services and overwhelm the target's inbox.
 2. Used as a means of protest or to demonstrate vulnerabilities.
 3. Vulnerability Demonstration:  Highlights weaknesses in email infrastructure.



Disadvantages:

 1. Highly illegal and unethical.
 2. Can result in severe legal consequences and damage relationships.
 3. Service Downtime: Can lead to significant downtime and inconvenience for the 
     targeted party.
"""

Application_text = """Applications:

1. Service Disruption: Overloading an email inbox, disrupting email services.
2. Demonstrating Vulnerabilities: Highlighting weaknesses in email infrastructure.
3. Misuse: Illegally causing service disruptions for personal gain or protest.
"""
