...
label = tk.Label(window, text="CHANGE MAC ADDRESS", font=('Inter', 30, 'bold'), fg='#000f46')
...
def open_specific_file2():
    ...
    # Specify the path to the Python script you want to open
    file_path = "D:\\Multipurpose Cybersecurity Tool\\Tools\\ChangeMacAddress.py"
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Introduction_text = """A Change MAC Address tool is a software application or command-line utility 
designed to modify the Media Access Control (MAC) address of a network 
interface on a computer or network device. The MAC address is a hardware 
identifier assigned to network interfaces for communication on a network. 
The tool allows users to change this address, providing a level of anonymity 
or security. While it can be used for legitimate purposes like 
troubleshooting or network testing, it's important to note that changing 
MAC addresses can also be misused for malicious activities, such as 
circumventing network access controls. Consequently, some jurisdictions 
regulate or restrict the use of MAC address-changing tools to prevent 
illicit activities.

Think of your device's MAC address as its unique ID card for connecting to the 
internet. A Change MAC Address tool is like a disguise kit for your device—it 
lets you change the ID card temporarily. 

It's handy for privacy or security reasons, as it makes it a bit harder for 
others to track your online activities. It's like wearing a different name tag at 
a party!
"""

Steps_text = """STEP 1: Click on the Change MAC Address tool button, a new window will pop up 
             on screen and check the admin privileges and network connection.

STEP 2: It will ask to Enter MAC address which need to be update.

STEP 3: A display list will be provided out of which user should select the option.

STEP 4: A new display list containing different MAC address will be displayed 
             from that user should select the IP address for the replacement, 
             enter the selected option.

STEP 5: It will the output if it is successfully changed or not.

STEP 6: To reenable the devices simply enter Y or y.
"""

Insights_text = """Advantages:

1. Helps in maintaining anonymity or bypassing certain network restrictions.
2. Can be used for troubleshooting network issues.
3. Troubleshooting: Helps in diagnosing network issues related to MAC addresses.



Disadvantages:

1. Unauthorized MAC address changes can breach network policies.
2. May not guarantee complete anonymity and can be detected.
3. Security Concerns: Misuse can lead to security breaches or unauthorized access.
"""

Application_text = """Applications:

1. Anonymity: Masking device identity for privacy or security reasons.
2. Network Bypass: Evading certain network restrictions or controls.
3. Troubleshooting: Diagnosing network issues related to MAC addresses.
"""

Result = PhotoImage(file= 'D:\\Multipurpose Cybersecurity Tool\\UI\\03_tools manual\\Result_Images\\MAC Address Change3.png')
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
