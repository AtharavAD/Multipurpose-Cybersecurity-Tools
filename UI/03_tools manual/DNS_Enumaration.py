...
label = tk.Label(window, text="      DNS ENUMERATION", font=('Inter', 30, 'bold'), fg='#000f46')
...
def open_specific_file2():
    ...
    # Specify the path to the Python script you want to open
    file_path = "D:\\Multipurpose Cybersecurity Tool\\Tools\\N_DNS.py"
...
1. Discovers DNS-related information like hostnames, IP addresses, etc.
2. Helps in mapping network infrastructure and identifying vulnerabilities.
3. Troubleshooting: Assists in diagnosing DNS-related network issues.



Disadvantages:

1. Limited by the security measures in place, may not reveal all details.
2. Unauthorized enumeration can be detected and blocked by security systems.
3.  Privacy Concerns: May expose sensitive network information if misused.
"""

Application_text = """Applications:

1. Network Mapping: Mapping network infrastructure by discovering DNS-related information.
2. Vulnerability Identification: Identifying potential vulnerabilities within DNS configurations.
3. Troubleshooting: Diagnosing and resolving DNS-related network issues.
"""

Result = PhotoImage(file= 'D:\\Multipurpose Cybersecurity Tool\\UI\\03_tools manual\\Result_Images\\DNS.png')
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
